...
$AppName          = "Install-2-Path, the installer script"
...
Write-Host "$AppName Installer v$AppVersion`n" -ForegroundColor Magenta
...
try {
    ...
    # Garante que $oldPath não seja nulo para evitar erro no split
    ...
    if ($pathClean -notcontains $finalBinPath) {
        ...
        # Garante que a pasta de backup existe antes de tentar salvar
        ...
        # Salva o backup. Se $oldPath for vazio, cria arquivo vazio.
        $oldPath | Out-File $backupFile -Encoding UTF8 -Force
        
        # Monta o novo PATH garantindo que não existam delimitadores duplicados
        $newPath = ($pathClean + $finalBinPath) -join ';'
        
        [Environment]::SetEnvironmentVariable("Path", $newPath, $scope)
        Write-Host "PATH updated successfully." -ForegroundColor Cyan
    } else {
        Write-Host "Path already exists in $scope. Skipping update." -ForegroundColor Cyan
    }
} catch {
